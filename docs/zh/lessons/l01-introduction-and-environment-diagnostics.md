...
> **硬件约定：**本讲以及 L01–L07 的最小实验可以使用 CPU 完成。AMD ROCm GPU 是
...
- L01–L07 学习环境、仿真、控制、相机、并行环境和抓取场景；
- L08–L11 生成演示、整理训练数据，并改变训练分布；
- L12–L13 训练策略，再通过闭环 rollout 检验任务结果。
...
没有可见 GPU 并不代表 L01 失败。CPU 学习者可以继续完成 L01–L07 的最小路径；后续
完整训练课程会单独说明更高的硬件要求。
...
| LeRobot 0.6.0 已安装 | 后续训练环境的提前提示；L01–L07 不要求 |
