...
> **项目状态：Alpha / 课程开发阶段。** L01–L07 已通过各自的 CPU clean-kernel 验证，
...
> `cpu-verified`。L11 已通过双语诊断、CPU+EGL 和参考 R9700 AMD+EGL 的 bounded DR 录制、
> 回读与 provenance 验证，状态为 `gpu-verified`。L13 仍为 `planned`。页面和 notebook 已建立
> 不代表对应课程已经完成验证。
...
2. 机器人自由度、关节控制、逆运动学、末端位姿、相机和并行仿真；
...
| L05 | [逆运动学、末端位姿与相机](docs/zh/lessons/l05-inverse-kinematics-end-effector-poses-and-cameras.md) | 120 分钟 | `cpu-ok` | `cpu-verified` |
| L06 | [并行仿真与批量 Franka 控制](docs/zh/lessons/l06-parallel-simulation-and-batched-franka-control.md) | 90 分钟 | `cpu-ok` | `cpu-verified` |
...
| L11 | [域随机化](docs/zh/lessons/l11-domain-randomization.md) | 90 分钟 | `gpu-recommended` | `gpu-verified` |
...
一个最小 Genesis 场景。CPU fallback 不阻止继续 L01–L07；训练环境要求和参考平台见
